...
export const createAvailabilityExceptionSchema = z.object({
    ...
    if(data.type!== 'BLOCK_FULL_DAY'){
        ...
        if(!data.endTime){
            ctx.addIssue({path: ['endTime'], code: 'custom', message: "End time is required for a no-full day exception"})
        }
...
export type CreateAvailabiltyRuleDto = z.infer<typeof createAvailabilityRuleSchema>;
export type UpdateAvailabiltyRuleDto = z.infer<typeof updateAvailabilityRuleSchema>;
export type CreateAvailabiltyExceptioneDto = z.infer<typeof createAvailabilityExceptionSchema>;
export type UpdateAvailabiltyExceptionDto = z.infer<typeof updateAvailabilityExceptionSchema>;